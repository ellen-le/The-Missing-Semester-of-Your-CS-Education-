...
# Run tests until one fails
RUN=1
while "$@" > "$LOGFILE" 2>&1; do # change this line to accpect command
    echo "Run $RUN passed" 
    ((RUN++))
done
...
sort直接使用，可以用于排序
- sort用于排序
uniq -c uniq用于去重，-c用与数数 
- 统计出现次数用 uniq
sort -nr -n按数字排序，-r大的在上面
- sort用于从大到小给数字排序
- 找文件用find
- find中name是用来找名字的 type f是用来找文件的
答案:find ~ -type f | sed 's/.*\.//' | grep -v '/' | sort | uniq -c | head -5
...
xargs 的功能是把前面管道传入文字切成一块块变成后面命令的参数 
![alt text](image-5.png)
xargs -0的作用是用于防止空格符的作用，例子如下：  
`find /tmp -name core -type f -print | xargs /bin/rm -f`
...
题目答案: find ~ -name "*.sh" -print0 | xargs -0 wc -l

### 使用 curl 获取 课程网站 的 HTML，并通过 grep 统计列出了多少讲。（提示：找出每讲课程名称在那份 HTML 中的共性；用 curl -s 关闭进度输出。）(https://missing.csail.mit.edu/)
答案:curl -s https://missing.csail.mit.edu/ | grep -c 'href="/2026/' 
单引号作用使双引号只作为双引号在这里存在
...
json（js object ）的作用适用于数据交换，一种轻量级的、纯文本的、人类和机器都能读懂的数据排版格式。{}对象[]列表数组
jq就是处理json的工具，该网站有一堆json数据。
- `.`表示所有内容 `.key` 提取keyvalue对应的value `.[]`提取数组内的东西 select负责筛选`.[]`传过来的的数据，其实就是流水线任务，看提示就能看懂
